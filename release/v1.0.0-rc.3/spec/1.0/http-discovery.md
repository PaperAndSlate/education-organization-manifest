...
The reference consumer follows at most five redirects by default, records each hop, rejects loops, userinfo, non-HTTP(S) schemes, private or reserved DNS answers, non-default ports, and missing JSON content types. It rechecks the destination before every hop, does not forward cookies or authorization, enforces a response byte limit and timeout, and requires HTTPS for every redirect destination. Cross-origin redirects are retained as observable metadata; they do not replace the original origin authority.
