...
Documentation/playground previews escape text, sanitize a narrow rich-text subset, reject unsafe URL schemes, use CSP where deployed, and isolate remote content. Browser signature verifiers MUST decode protected-header bytes as fatal UTF-8 and reject headers over their configured byte budget before JSON parsing. Agent inputs are untrusted evidence; source text cannot instruct an agent to bypass safety or use tools. Supply-chain controls include lockfiles, review, dependency/license scanning, pinned CI actions, SBOM, provenance, and protected release environments.
